...
// the text renders at a FIXED height, CENTERED on the path, shrinking only if it would overrun. The path
// controls placement + curvature, NOT size. The curvature belongs to the hat, not the text.
//
...
export function pathTextLayout(p0: Pt, control: Pt, p2: Pt, glyphWidths: number[], spacingPx: number): PathTextLayout {
  ...
  // FIXED letter height: the text renders at its given size, CENTERED on the arc — the arc controls
  // placement + curvature, not size. Only shrink when the text would overrun the path (long words). Short
  // words sit centered at full size, not ballooned to fill — the size comes from a set per-zone height.
