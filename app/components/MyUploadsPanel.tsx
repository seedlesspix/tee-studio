  ...
  if (horizontal) {
    return (
      <div className="flex min-h-0 flex-1 items-center gap-2 overflow-x-auto pb-1">
        {loading ? (
          <span className="px-2 text-xs text-gray-400">{t('designer.upload.loading', 'Loading…')}</span>
        ) : uploads.length === 0 ? (
          <span className="px-2 text-xs text-gray-400">{t('designer.upload.empty_mobile', 'No uploads yet — add one above.')}</span>
        ) : (
          uploads.map(item => (
            <div key={item.id} className="relative h-16 w-16 shrink-0 rounded-lg border border-gray-200 bg-white">
              <button
                type="button"
                onClick={() => onPick(item)}
                title={`${t('designer.upload.tile_add', 'Add')} "${item.fileName}"`}
              ...
              <button
                ...
                title={t('designer.upload.remove_tooltip', 'Remove from My Uploads')}
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center justify-between">
        <label className="text-xs text-gray-800 uppercase tracking-widest font-mono">{t('designer.upload.heading', 'My Uploads')}</label>
        {uploads.length > 0 && (
      ...
      {loading ? (
        <p className="mt-3 text-xs text-gray-400 font-mono text-center py-4">{t('designer.upload.loading', 'Loading…')}</p>
      ) : uploads.length === 0 ? (
        <div className="mt-2 border border-dashed border-gray-200 rounded-xl px-4 py-6 text-center">
          <p className="text-sm text-gray-500">{t('designer.upload.empty_title', 'No uploads yet')}</p>
          <p className="text-[11px] text-gray-400 mt-1">{t('designer.upload.empty_hint', 'Images you upload appear here to reuse on any design.')}</p>
        </div>
      ) : (
          ...
          {uploads.map(item => (
            <div key={item.id}
              ...
              <button
                type="button"
                onClick={() => onPick(item)}
                title={`${t('designer.upload.tile_add', 'Add')} "${item.fileName}" ${t('designer.upload.tile_add_suffix', 'to your design')}`}
                ...
                <span className="absolute inset-0 flex items-center justify-center bg-[#dd3333]/85 opacity-0 group-hover:opacity-100 transition-opacity text-white text-[11px] font-bold uppercase tracking-wide">
                  {t('designer.upload.add_overlay', '+ Add')}
                </span>
              </button>
