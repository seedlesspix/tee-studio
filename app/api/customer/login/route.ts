...
// Node runtime — we use node:crypto for PKCE and randomness. Matches the
// existing /api/cart-add precedent.
export const runtime = 'nodejs'

// GET /api/customer/login?return_to=<path>
//
// Initiates the Shopify Customer Account API OAuth flow. Generates a PKCE
// verifier, an OAuth state (CSRF), and an OIDC nonce (replay), stashes
// them in short-lived HttpOnly cookies, then 302s the browser to the
// discovered authorization endpoint. /auth/customer/callback validates
// state, swaps the code for tokens, and consumes the cookies.
//
// Day 2 scope: redirect only. Day 5 will swap this to POST and accept a
// canvas snapshot, write a draft row to design_orders, and stash the UUID
// in cust_oauth_draft_id for /auth/customer/callback to pick up.
export async function GET(request: NextRequest) {
  ...
  response.cookies.set('cust_pkce_verifier', pkceVerifier, flowCookieOpts)
